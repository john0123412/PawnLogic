...
# ════════════════════════════════════════════════════════
# 会话自动命名：轻量模型优先链
# 后台命名线程按顺序尝试，首个 API Key 有效的模型中选
# 全链不可用时降级到当前会话的模型
# ════════════════════════════════════════════════════════
NAMING_MODEL_CHAIN: list = [
    "claude-haiku",   # Anthropic Haiku — 极速响应、廉价
    "ds-chat",        # DeepSeek V3 — 性价比兜底
    "glm-4.5-air",    # 智谱轻量 — 国内直连
    "qwen-turbo",     # 通义极速
    # None/空列表末尾 → fallback 到 session.model_alias（现有行为）
]
