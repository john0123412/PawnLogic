...
| `core/agent_orchestrator.py` | Bounded orchestration Interface | `SerialAgentOrchestrator`, `CancellationToken`, `BudgetLedger` | `test_agent_orchestrator.py`, `test_delegation_contracts.py` | Tasks execute in input order through an injected Delegation Runtime executor. Budget claims are atomic and single-settlement; concurrency above one fails closed until Workspace and RuntimeContext isolation is proven. |
