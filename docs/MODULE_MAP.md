...
| `core/agent_orchestrator.py` | Bounded orchestration Interface | `SerialAgentOrchestrator`, `CancellationToken`, `BudgetLedger` | `test_agent_orchestrator.py`, `test_concurrent_delegation.py` | Tasks retain input-order results through an injected Delegation Runtime executor. Atomic claims are single-settlement; one or two workers only, with task-local cancellation and deadlines. A two-worker executor must be concurrency-safe. |
...
| `pawnlogic/live_terminal.py` | Persistent terminal owner | `PersistentTerminal`, `PersistentTerminalController`, `TerminalSink` | `test_live_terminal.py`, `test_e2e.py` | One Prompt Toolkit application renders streamed output above muted queue rows, a fixed composer, and a toolbar; mouse-wheel events own an independent output scroll anchor, recovered drafts carry explicit replacement intent, worker stdout/stderr never write directly to the physical TTY, and modal pause/resume restores terminal ownership exactly once. The 0.3.7 cycle drops `full_screen` / `mouse_support` and routes all output through a single `TerminalTranscript`. |
| `pawnlogic/terminal_transcript.py` | Single-owner transcript buffer for the persistent terminal | `TerminalTranscript`, `TextSink`, `normalize_sink` | `test_live_terminal_inline.py`, `test_model_selector_modal.py` | Thread-safe bounded in-memory buffer with a pluggable host sink; recovery-draft marker; `append` / `flush` / `snapshot` / `drain` / `replace`. Replaces the legacy parallel `_output_chunks` deque so every writer (sink, stdout/stderr proxy, app output render) routes through the same transcript. Added in 0.3.7. |
