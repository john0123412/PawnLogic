...
| `pawnlogic/live_repl.py` | Prompt Toolkit live-composer Adapter | `build_prompt_toolkit_bindings()`, `build_bottom_toolbar()`, `dispatch_live_input()` | `test_e2e.py`, `test_live_repl.py`, `test_cli_startup.py` | Typed Enter/Alt+Enter and Alt+Up recall submissions, immutable queue toolbar status, and safe live slash-command gating stay separate from the CLI startup facade; readline remains serial. |
