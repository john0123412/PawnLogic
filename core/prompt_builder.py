...
def build_session_prompt(
    *,
    ...
    prompt = (
        ...
        "    2. The first <tool_call> must immediately follow </plan>.\n"
        "    3. Do not insert explanatory text, reasoning, or apologies between </plan> and the "
        "first <tool_call>.\n"
        "    4. Emit each tool call in its own <tool_call>...</tool_call> block.\n\n"
        ...
        "Use exactly this text tool-call format:\n"
        "  <tool_call>{\"name\":\"tool_name\",\"arguments\":{\"key\":\"value\"}}</tool_call>\n\n"
        "Rules:\n"
        "  - Every tool call MUST be a <tool_call> block containing one JSON object.\n"
        ...
        "NO WAITING RULE: Output the <plan> block and the <tool_call> in the same response.\n"
        "Do not output a plan and then wait for user confirmation.\n\n"
        ...
        "    </plan>\n"
        "    [re-emit the original tool call in the required <tool_call> JSON envelope]\n\n"
