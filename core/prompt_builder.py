...
def build_session_prompt(
    *,
    ...
    prompt = (
        ...
        "  WHEN OUTPUTTING BOTH A <plan> BLOCK AND TOOL CALLS:\n"
        "    1. The <plan> block MUST come FIRST in your response\n"
        "    2. Tool calls MUST IMMEDIATELY FOLLOW the closing </plan> tag\n"
        "    3. Do NOT insert explanatory text, reasoning, or apologies between </plan> and the first tool call\n"
        "    4. Valid formats:\n"
        "       a) <plan><intent>...</intent></plan><call name=\"tool_name\">...</call>\n"
        "       b) <plan><intent>...</intent></plan>3.14{\"tool\":\"tool_name\",\"args\":{...}}\n"
        "    5. INVALID (will cause parsing failure):\n"
        "       a) <plan><intent>...</intent></plan>[any text here]<call...>\n"
        "       b) <plan><intent>...</intent></plan>\\n\\n[explanation]\\n<call...>\n"
        "       c) Any tool call format that cannot be parsed by the hybrid XML/JSON parser\n\n"
        "  User Input → [YOU] → <plan> parser → tool executor → result injector → [YOU again]\n"
        "The <plan> tag anchors your intent across long tasks. A single short line is enough:\n"
        "  <plan><intent>what to do next and why</intent></plan>\n"
        "Tool calls emitted without <plan> are flagged and trigger a self-correction signal.\n\n"
