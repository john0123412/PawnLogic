...
PawnLogic 是一个终端优先的自主 AI Agent，支持多 Provider 模型路由、持久化记忆、真实本地工具执行、MCP 集成和面向 CTF 的工具链。当前源码版本是 **0.1.2**；发布到 PyPI 和 GitHub Releases 需要在 release approval 后进行。
...
只有在上游许可证和 notice 已完成再分发审查后，才会随 PyPI 分发。
...
- `pawnlogic[ctf]` 明确表示只安装 CTF 工具依赖；CTF skill pack 仍是用户安装的扩展资产，除非再分发许可证已完成审查。
- `/ctf init`、`/ctf status`、`/ctf artifact`、`/ctf remote`、`/ctf flag` 和 `/ctf writeup` 会在当前 workspace 管理轻量 challenge metadata；flag candidate 只有通过 `/ctf solved` 显式确认后才视为 solved。
- CTF writeup 草稿会基于 workspace metadata 和 session evidence 导出为 Markdown。
- 现有 JSONL tool audit 可附加可选 CTF metadata，不改变基础 audit 格式。
- 第三方 CTF skill-pack 候选项记录在 `THIRD_PARTY_NOTICES.md` 中，在许可证审查完成前会从 PyPI 分发包和生成的 release source archive 中排除。
