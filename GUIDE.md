...
Web fetch and browser navigation accept HTTP(S) targets only. PawnLogic rejects
embedded URL credentials, cloud-metadata/internal hosts, and loopback,
link-local, multicast, unspecified, or reserved addresses. Private-network
targets require a real host confirmation; tool arguments generated by the model
cannot grant that authorization. Non-interactive execution fails closed.

Every HTTP redirect and browser final URL is evaluated again. A confirmed
private target is fetched directly and is never sent through the remote Jina
Reader service. Docker `bridge`/`host` networking and legacy
`uvx mcp-server-fetch` startup have separate capability-only gates:
`allow_network` / `PAWNLOGIC_DOCKER_ALLOW_NETWORK` and
