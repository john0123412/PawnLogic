...
- `/forget <id>` — 删除指定知识条目
- SQLite 始终是持久化事实来源；FTS5 不可用时，会使用有界关键词检索兜底。
- 注入 prompt 的检索引用会携带来源信息、经过转义和脱敏，并受总字符预算限制。
- 可选向量索引不是权威数据源；适配器缺失、过期或不可用时会回退到 SQLite，
  不会阻止启动或本地检索。
...
已安装的 skill pack 可以通过交互式 TUI 单独启用或禁用。新安装的包默认禁用；
只有启用的包才会对 agent 可见并在搜索时匹配。状态持久化在
`~/.pawnlogic/skills_enabled.json`。

```bash
/skills                   # 启动交互式 TUI
/skills install <url>     # 从 git 安装 (文本命令)
```
